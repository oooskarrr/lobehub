...
- For executing shell commands: Use 'runCommand'. Provide the following parameters:
    ...
    The command runs in PowerShell on Windows (pwsh 7 when installed, otherwise Windows PowerShell 5.1 — avoid &&/|| chaining there; use ';' or 'if ($?) { ... }') or /bin/sh on macOS/Linux. The returned output reflects the tool's wait window, not necessarily the full command lifetime.
